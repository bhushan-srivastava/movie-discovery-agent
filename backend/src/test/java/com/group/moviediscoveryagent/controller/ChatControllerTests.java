...
class ChatControllerTests {
    ...
    void conversationNotFound_returns404() throws Exception {
        ...
        mvc.perform(post("/api/conversations/" + UUID.randomUUID() + "/chat")
    ...
    void blankMessage_returns400() throws Exception {
        ...
        mvc.perform(post("/api/conversations/" + conv.getId() + "/chat")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(req)))
    ...
    @Test
    void completedJsonResponseReturned() throws Exception {
        var conv = new Conversation();
        conv.setId(UUID.randomUUID());
        ...
        var result = mvc.perform(post("/api/conversations/" + conv.getId() + "/chat")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(mapper.writeValueAsString(req)))
                .andExpect(status().isOk())
                .andReturn();
        ...
        assertThat(mapper.readTree(body).get("message").asText()).isEqualTo("Hello from test model");
    }
